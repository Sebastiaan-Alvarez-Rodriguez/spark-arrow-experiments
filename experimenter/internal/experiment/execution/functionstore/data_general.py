...
def deploy_data_default(interface, idx, num_experiments, nodes):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=cpu_count()-1) as executor:
        ...
        futures_connection = {x: executor.submit(_get_ssh_connection, x.ip_public, silent=silent, ssh_params=_merge_kwargs(ssh_kwargs, {'User': x.extra_info['user']})) for x in nodes}
