        ...
        for mode in ['--arrow-only', '--spark-only']:
            for selectivity, data_query in zip(row_selectivities, data_queries):
                for stripe in object_sizes:
                    ...
                    configbuilder.set('runs', 6)  # Very low amount of repeats to account for huge search space (42 experiments). Data is stable anyways.
                    ...
                    configbuilder.set('remote_result_dir', fs.join('~', 'results', 'exp_objectsize_selectivity', str(timestamp), result_dirname))
                    configbuilder.set('result_dir', fs.join(loc.result_dir(), 'exp_objectsize_selectivity', str(timestamp), result_dirname))
