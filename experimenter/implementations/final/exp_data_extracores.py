...
class CephExperiment(ExperimentInterface):
    ...
    def get_executions(self):
        ...
        multipliers = [(32, 4), (32, 8), (32, 16), (32, 32), (32, 64), (32, 128)] #Total data size: 16, 32, 64, 128, 256, 512GB
        ...
        for mode in ['--arrow-only', '--spark-only']:
            for (copy_multiplier, link_multiplier) in multipliers:
                ...
                configbuilder.set('remote_result_dir', fs.join('~', 'results', 'exp_data_extracores', str(timestamp), result_dirname))
                configbuilder.set('result_dir', fs.join(loc.result_dir(), 'exp_data_extracores', str(timestamp), result_dirname))
