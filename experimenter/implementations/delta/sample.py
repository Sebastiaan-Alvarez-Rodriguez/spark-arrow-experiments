...
# A sample experiment, showing how to start implementing Delta Lake experiments.
class CephExperiment(ExperimentInterface):
    ...
    def get_executions(self):
        ...
        multipliers = [(32, 4)] #Total data size: 16GB
        ...
        for mode in ['--arrow-only', '--spark-only']:
            for (copy_multiplier, link_multiplier) in multipliers:
                ...
                configbuilder.set('spark_conf_options', lambda conf: ExperimentConfiguration.base_spark_conf_options(conf)+[
                    ...
                    "'spark.sql.extensions=org.arrowspark.spark.sql.write.ArrowWriteExtension,io.delta.sql.DeltaSparkSessionExtension'", # <--- This is important for Delta and our connector.
                    "'spark.sql.catalog.spark_catalog=org.apache.spark.sql.delta.catalog.DeltaCatalog'", # <----------- This is important for Delta too.
                ])
