...
class CephExperiment(ExperimentInterface):
    ...
    def get_executions(self):
        ...
        multipliers = [(16, 2)] #Total data size: 32GB
        ...
        for row_selectivity, data_query in zip(row_selectivities, data_queries):
            for mode in modes:
                for (copy_multiplier, link_multiplier) in multipliers:
                    ...
                    configbuilder.set('remote_result_dir', fs.join('~', 'results', 'exp06_bigfile', result_dirname, str(timestamp)))
                    configbuilder.set('result_dir', fs.join(loc.result_dir(), 'exp06_bigfile', result_dirname, str(timestamp)))
