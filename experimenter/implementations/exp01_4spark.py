        ...
        for row_selectivity, data_query in zip(row_selectivities, data_queries):
            for mode in modes:
                for (copy_multiplier, link_multiplier) in multipliers:
                    ...
                    configbuilder.set('remote_result_dir', fs.join('~', 'results', 'exp01_4spark', result_dirname, str(timestamp)))
                    configbuilder.set('result_dir', fs.join(loc.result_dir(), 'exp01_4spark', result_dirname, str(timestamp)))
