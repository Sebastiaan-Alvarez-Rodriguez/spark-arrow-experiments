...
class CephExperiment(ExperimentInterface):
        ...
        for row_selectivity, data_query in zip(row_selectivities, data_queries):
            for mode in modes:
                for (copy_multiplier, link_multiplier) in multipliers:
                    ...
                    configbuilder.set('remote_result_dir', fs.join('~', 'results', 'exp01', result_dirname, str(timestamp)))
                    configbuilder.set('result_dir', fs.join(loc.result_dir(), 'exp01', result_dirname, str(timestamp)))
    ...
    def get_optimizationconfig(self):
        ...
        priority_options = { # Note: Highest priority goes first. Also note: Priorities must be unique to ensure consistent behaviour.
            DataOptimization(): 1,
            DistributionOptimization(keys=['rados_ceph', 'spark']): 2,
        }
        return OptimizationConfig(optimizations=list(priority_options.keys()), optimizations_comperator=lambda x,y: priority_options[x] > priority_options[y])
